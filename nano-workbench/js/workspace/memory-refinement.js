...
const MAX_SOURCE_PROMPT_CHARS = 12000;
const MAX_PARENT_PROMPT_CHARS = 4000;
...
export function clipMemorySource(text, maxChars) {
...
export function buildMemoryRefinementPrompt({ mode, sourceText, parentText = '', projectName = '' }) {
  const source = clipMemorySource(sourceText, MAX_SOURCE_PROMPT_CHARS);
  const parent = clipMemorySource(parentText, MAX_PARENT_PROMPT_CHARS);
  const project = String(projectName || '').trim().slice(0, 200);
