...
- 前後の助詞・句読点・敬称は個人情報そのものではない限り含めない。例: 「西村様」なら text は「西村」。
- entities は文書内の出現順に返す。
- 同じマスク対象文字列が複数回出る場合、各出現をそれぞれ返す。
- 原文に存在しない文字列は絶対に返さない。
- 法人名、官公庁名、部署名、一般的な地名、金額、製品名は、それ自体が個人識別情報でなければ返さない。
- メールの From/To/Cc/差出人/宛先にある個人の表示名と、その個人にひも付くメールアドレスは PERSON / EMAIL とする。
- 署名の氏名、本文中の「姓+さん/さま/様/氏/先生」、個人を指す姓のみの呼称も PERSON とする。
- 日本語名とローマ字名が併記されて同一人物を示す場合、可能なら併記部分全体を PERSON として原文どおり返す。
- 文献の発行年、出典年、送信日時、会議日、締切、ファイル名中の日付やバージョンは、それだけでは個人情報ではない。standard モードでは OTHER や DOB として返さない。
- DOB は、原文の文脈で明確に個人の生年月日・誕生日・生年を示す場合だけ使う。
